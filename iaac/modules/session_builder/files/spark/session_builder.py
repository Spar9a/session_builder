...
SESSION_TIMEOUT_SECS = 5 * 60  # 5 minutes in seconds
LOOKBACK_DAYS = 5
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--processed_data_folder', required=True, help='Folder for processed data')
...
DELTALAKE_TABLE_FOLDER = args.deltalake_folder

DELTA_TABLE_PATH = f"s3://{DATA_LAKE_BUCKET}/{DELTALAKE_TABLE_FOLDER}"
RAW_DATA_FOLDER_PATH = f"s3://{DATA_LAKE_BUCKET}/{RAW_DATA_FOLDER}"
...
def move_files(files: list[str]):
    ...
    try:
        s3 = boto3.client('s3')
        for f in files:
            bucket_name, object_key = f.replace("s3://", "").split("/", 1)
